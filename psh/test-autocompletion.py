...
def harness(p):
    ...
    assert_completion(p, [['e', 'tc/']])
    assert_completion(p, [['/e', 'tc/']])
    assert_completion(p, [['/etc', '/']])

    assert_completion(p, [['testen', 'v/']])
    assert_completion(p, [['/testen', 'v/']])
    assert_completion(p, [['testenv/i', 'psum/']])

    longpath = 'ls testenv/lorem/./../lorem/../../testenv/ip'
    assert_completion(p, [[longpath, 'sum']])
    assert_completion(p, [['../testenv/ip', 'sum']])
    ...
    assert_completion(p, [['testen', 'v/'], ['loremi', 'psum/'], ['dol', 'or.txt']])
